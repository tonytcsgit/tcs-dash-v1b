...
class OperationsBrowser(unittest.TestCase):
    def test_demo_layout_metrics_navigation_and_no_live_requests(self):
        ...
        with sync_playwright() as pw:
            ...
            self.assertIn('DEMO — NOT LIVE OPERATIONS',page.inner_text('body'))
            current=page.locator('#lawsuit-priorities li').all_text_contents()
            self.assertEqual(current, ['Olympus — BP + Parker', 'SMA — Parker + Pulaski', 'CHLOR — BP + DIF', 'Rideshare — BP'])
            self.assertIn('As of September 24',page.locator('#lawsuit-priorities').inner_text())
            self.assertIn('Lawsuit priorities updated September 24',page.locator('.demo-banner').inner_text())
            self.assertIn('upcoming budget context remains from September 12',page.locator('.demo-banner').inner_text())
            self.assertEqual(page.locator('#ops-results > section').evaluate_all('(nodes) => nodes.map(n => n.id)'), ['live-programs', 'upcoming-launches', 'buyer-health', 'prospects', 'needs-attention'])
            delivery_box=page.locator('#live-programs').bounding_box()
            self.assertGreater(page.locator('#upcoming-launches').bounding_box()['y'],delivery_box['y']+delivery_box['height'])
            upcoming=page.locator('#upcoming-launches .ops-launch')
            self.assertEqual(upcoming.locator('h3').all_text_contents(), ['OLYMPUS', 'Chlorpyrifos', 'Dupixent', 'Apple AirTag'])
            self.assertEqual(upcoming.locator('.eyebrow').all_text_contents(), ['Priority 1', 'Priority 2', 'Priority 3 — equal', 'Priority 3 — equal'])
            self.assertEqual(upcoming.locator('.context-buyer').all_text_contents(), ['Parker']*4)
            self.assertNotIn('Sep 18',page.locator('#upcoming-launches').inner_text())
            self.assertIn('Not set',page.locator('#upcoming-launches').inner_text())
            ...
            self.assertEqual(page.locator('#upcoming-launches .ops-launch').count(),4)
